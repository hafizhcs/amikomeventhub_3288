...
<section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-16">
  ...
  <div class="flex-1 space-y-8 text-center md:text-left">
    ...
    <p class="text-lg text-slate-500 max-w-lg leading-relaxed mx-auto md:mx-0">
  ...
  <div class="relative flex-1">
    ...
    <img 
      ...
      alt="Hero Image" 
      class="w-full h-auto rounded-[2rem] object-cover shadow-2xl"
    >

    <!-- Card verifikasi -->
    <div class="absolute -bottom-6 left-6 right-6 md:left-[-1.5rem] md:right-auto z-20 
                bg-white/90 backdrop-blur-xl p-6 rounded-3xl shadow-2xl border border-white">
...
<!-- ==================== SECTION EVENT SELESAI / HABIS ==================== -->
@if($pastEvents->isNotEmpty())
<section id="past-events" class="max-w-7xl mx-auto px-6 py-20 border-t border-slate-100">
    <!-- Header Section: Disamakan dengan layout Partner Kami -->
    <div class="flex justify-between items-end mb-10">
        <div>
            <h2 class="text-3xl font-extrabold mb-2">Event Terdahulu & Habis Terjual</h2>
            <p class="text-slate-500 font-medium">Kumpulan keseruan event yang telah selesai dilaksanakan dan tiket yang sudah habis.</p>
        </div>
    </div>

    <!-- Grid Layout Kartu Event: Menyesuaikan gap-8 dan responsivitas -->
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        @foreach($pastEvents as $past)
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg transition overflow-hidden flex flex-col opacity-75 group">
                
                <!-- Area Gambar Non-Aktif -->
                <div class="relative aspect-[4/3] w-full overflow-hidden bg-slate-100">
                    <img src="{{ $past->poster_path ? asset('storage/' . $past->poster_path) : 'https://placehold.co/600x800' }}" 
                         alt="{{ $past->title }}" 
                         class="w-full h-full object-cover grayscale filter brightness-75">
                         
                    <span class="absolute top-3 left-3 bg-slate-900/80 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">
                        @if($past->stock <= 0) Habis Terjual @else Selesai @endif
                    </span>
                </div>

                <!-- Konten Informasi Card (p-6 agar sejajar dengan isi partner) -->
                <div class="p-6 flex flex-col flex-grow">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">
                        {{ $past->category->name ?? 'Event' }}
                    </span>
                    
                    <h3 class="font-bold text-slate-700 text-base leading-snug line-clamp-2 mb-2">
                        {{ $past->title }}
                    </h3>
                    
                    <p class="text-xs text-slate-400 font-medium mb-4 mt-auto flex items-center gap-1.5">
                        <svg class="h-3.5 w-3.5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        {{ \Carbon\Carbon::parse($past->date)->translatedFormat('d M Y') }}
                    </p>
                    
                    <button disabled class="w-full py-2.5 px-4 bg-slate-100 text-slate-400 font-semibold text-xs rounded-xl cursor-not-allowed text-center">
                        Penjualan Ditutup
                    </button>
                </div>
...
<section id="partners" class="max-w-7xl mx-auto px-6 py-20">
    ...
    @if($topReviews->isNotEmpty())
        <section class="max-w-7xl mx-auto px-6 py-20">
            <div class="mb-10 text-center">
                <p class="text-sm font-semibold uppercase tracking-[0.3em] text-indigo-600">Apa Kata Mereka</p>
                <h2 class="mt-4 text-4xl font-extrabold text-slate-900">Ulasan 5 Bintang dari Peserta</h2>
                <p class="mt-4 text-slate-500 max-w-2xl mx-auto">Lihat pengalaman nyata peserta yang sudah hadir dan merekomendasikan event ini.</p>
            </div>

            <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach($topReviews as $review)
                    <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-lg transition">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-100 text-amber-700 font-black">★</div>
                            <div>
                                <p class="text-sm font-semibold text-slate-900">{{ $review->user->name ?? 'Pengguna' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->created_at->translatedFormat('d M Y') }}</p>
                            </div>
                        </div>
                        <div class="mb-4 flex items-center gap-1 text-amber-500 text-sm font-semibold">
                            @for ($i = 0; $i < 5; $i++)
                                <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.966a1 1 0 00.95.69h4.173c.969 0 1.371 1.24.588 1.81l-3.38 2.455a1 1 0 00-.364 1.118l1.287 3.966c.3.921-.755 1.688-1.54 1.118l-3.38-2.455a1 1 0 00-1.176 0l-3.38 2.455c-.784.57-1.838-.197-1.539-1.118l1.286-3.966a1 1 0 00-.364-1.118L2.05 9.393c-.783-.57-.38-1.81.588-1.81h4.173a1 1 0 00.95-.69l1.286-3.966z"/></svg>
                            @endfor
                            <span class="text-slate-600 ml-2">{{ $review->score ?? 5 }}/5</span>
                        </div>
                        <p class="text-slate-600 mb-6">{{ \Illuminate\Support\Str::limit($review->review ?? 'Tidak ada komentar tambahan.', 120, '...') }}</p>
                        <div class="flex items-center justify-between gap-3 text-xs text-slate-500">
                            <span>{{ $review->event->title ?? 'Event' }}</span>
                            <span class="rounded-full bg-slate-100 px-3 py-1">{{ $review->created_at->diffForHumans() }}</span>
                        </div>
                    </article>
                @endforeach
            </div>
    ...
    {{-- Testimonial section duplicate removed to fix layout --}}
