 ...
 <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
    ...
    @elseif($trx->status === 'pending' && $trx->expires_at && now()->lte($trx->expires_at))
    ...
    @elseif($trx->status === 'cancelled' || ($trx->expires_at && now()->gt($trx->expires_at)))
        <span class="px-3 py-1 bg-rose-100 text-rose-700 rounded-lg text-xs font-bold uppercase ring-1 ring-rose-200">Expired</span>
    @else
        <span class="px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-xs font-bold uppercase ring-1 ring-gray-200">{{ ucfirst($trx->status) }}</span>
    @endif
</td>
