...
<!-- ================= BAGIAN BARU: GRAFIK ANALISIS ================= -->
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
    <!-- Grafik Pertumbuhan Pengguna -->
    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm">
        <div class="mb-4">
            <h3 class="font-black text-lg text-slate-800">Pertumbuhan Pengguna Baru</h3>
            <p class="text-xs text-slate-400 font-medium">Tren pendaftaran akun pengguna setiap bulan</p>
        </div>
    ...
    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm">
        <div class="mb-4">
            <h3 class="font-black text-lg text-slate-800">Produktivitas Event</h3>
            <p class="text-xs text-slate-400 font-medium">Jumlah penyelenggaraan event baru yang terbit</p>
        </div>
        <div class="relative w-full h-72">
...
<!-- Scripts untuk Inisialisasi Grafik -->
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Fallback data jika array kosong (Passing data terformat JSON dari Controller)
        const months = {!! json_encode($chartLabels ?? ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun']) !!};
        const userData = {!! json_encode($chartUserData ?? [12, 19, 32, 45, 62, 85]) !!};
        const eventData = {!! json_encode($chartEventData ?? [5, 8, 12, 10, 15, 22]) !!};

        // 1. Inisialisasi Grafik Pertumbuhan Pengguna (Line Chart Premium)
        const ctxUser = document.getElementById('userGrowthChart').getContext('2d');
        
        // Membuat efek gradient warna ungu/indigo di bawah garis chart
        const userGradient = ctxUser.createLinearGradient(0, 0, 0, 300);
        userGradient.addColorStop(0, 'rgba(79, 70, 229, 0.3)');
        userGradient.addColorStop(1, 'rgba(79, 70, 229, 0)');
        ...
        new Chart(ctxUser, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    fill: true,
                    tension: 0.35 // Membuat lengkungan garis lebih smooth
                }]
            },
            options: {
                responsive: true,
        ...
        // 2. Inisialisasi Grafik Pertumbuhan Event (Bar Chart Modern)
        ...
        new Chart(ctxEvent, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderRadius: 8, // Membuat sudut batang melengkung modern
                    borderSkipped: false,
                    barThickness: 24
                }]
            },
