...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-800 mt-2">Buat Kupon Baru</h1>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 md:p-8">
        <form action="{{ route('admin.coupons.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="code" id="code" value="{{ old('code') }}" placeholder="Contoh: KUPONMABA" class="w-full px-4 py-2.5 border @error('code') border-rose-300 focus:ring-rose-200 focus:border-rose-400 @else border-slate-200 focus:ring-indigo-200 focus:border-indigo-400 @enderror rounded-lg shadow-sm focus:outline-none focus:ring-4 uppercase font-mono tracking-wider transition">
                @error('code')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="value" id="value" value="{{ old('value') }}" placeholder="Contoh: 15 atau 20000" min="1" class="w-full px-4 py-2.5 border @error('value') border-rose-300 focus:ring-rose-200 focus:border-rose-400 @else border-slate-200 focus:ring-indigo-200 focus:border-indigo-400 @enderror rounded-lg shadow-sm focus:outline-none focus:ring-4 transition">
                    @error('value')
                        <p class="mt-1.5 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition font-medium text-sm shadow-sm">Simpan Kupon</button>
            </div>
        </form>
    </div>
</div>
